...
int spike_fd_close(int fd) {
  spike_file_t *f = spike_file_get(fd);
  if (!f)
    return -1;
  spike_file_t *old = atomic_cas(&spike_fds[fd], f, 0);
  spike_file_decref(f);
  if (old != f)
    return -1;
  spike_file_decref(f);
...
ssize_t spike_file_pwrite(spike_file_t *f, const void *buf, size_t size, off_t offset) {
  return frontend_syscall(HTIFSYS_pwrite, f->kfd, (uint64)buf, size, offset, 0, 0, 0);
}
